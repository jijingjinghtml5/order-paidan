<template>
  <div class="img-frames">
    <img v-if="swiper" :src="url" @click="$emit('open-img')" />
    <slot v-else name="text" :zoomIn="zoomIn" />
    <div
      ref="box"
      :class="'image-check' + (open || show ? ' open' : '')"
      :style="{ right: rights, width: imageCheckWidth }"
    >
      <div class="texts">{{ text }}</div>
      <div class="box" :style="moveCss()">
        <img ref="img" :src="url" draggable="false" />
      </div>
      <div v-show="kaiguan" class="close" :style="{ right: rightclose }" @click="zoomOut">
        <div>
...
.texts {
  ...
  padding: 14px;
  border-radius: 5px;
  background-color: rgba(84, 90, 92, 0.4);
  z-index: 999999;
  color: rgba(210, 248, 243, 0.788);
  font-size: 18px;
  font-weight: 400;
}
</style>
