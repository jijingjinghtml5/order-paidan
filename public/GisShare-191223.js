...
function postToIframeMethed(methedName,data,backFun) {
    ...
    var win = document.getElementById('cd-map').contentWindow;
    //  var obj = { name: 'Jack' };
// 存入对象
...
function doGetGraphicByJson(jsonStr) {
    postToIframeMethed("doGetGraphicByJson",jsonStr);
}
function drawSquare() {
...
function doNewQuery(str,layerName){
    postToIframeMethed("newQuery",[str,layerName]);
}
function doGridQuery(str){
    doNewQuery(str,"责任网格");
}
